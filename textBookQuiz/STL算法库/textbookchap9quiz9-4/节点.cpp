...
int main() {
	...
	cout << "Initial list (size=" << list.getSize() << "): ";
	int n = list.getSize();
	if (n > 0) {
		list.moveToStart();
		for (int i = 0; i < n; ++i) {
			int v;
			list.getCurrentData(v);
			cout << v << " ";
			list.next();
		}
	}
	...
	// 移动游标并在当前前后插入
	list.moveToPos(2); // 把游标移到索引2
	cout << "Position after moveToPos(2): " << list.getPosition() << endl;
	int cur;
	list.getCurrentData(cur);
	cout << "Current value: " << cur << endl;

	list.insertBeforeCurrent(100);
	cout << "After insertBeforeCurrent(100): ";
	n = list.getSize();
	list.moveToStart();
	for (int i = 0; i < n; ++i) { int v; list.getCurrentData(v); cout << v << " "; list.next(); }
	cout << endl;

	list.insertAfterCurrent(200);
	cout << "After insertAfterCurrent(200): ";
	n = list.getSize();
	list.moveToStart();
	for (int i = 0; i < n; ++i) { int v; list.getCurrentData(v); cout << v << " "; list.next(); }
	cout << endl;
	...
	// 删除当前前后元素
	if (list.deleteBeforeCurrent()) {
		cout << "After deleteBeforeCurrent: ";
		n = list.getSize();
		list.moveToStart();
		for (int i = 0; i < n; ++i) { int v; list.getCurrentData(v); cout << v << " "; list.next(); }
		cout << endl;
	...
	if (list.deleteAfterCurrent()) {
		cout << "After deleteAfterCurrent: ";
		n = list.getSize();
		list.moveToStart();
		for (int i = 0; i < n; ++i) { int v; list.getCurrentData(v); cout << v << " "; list.next(); }
		cout << endl;
	...
	// 清空链表
	list.clear();
	cout << "After clear, size = " << list.getSize() << endl;
