...
void SettingsWidget::initUI()
{
    ...
    QLabel *frameRateTitleLabel = new QLabel("Frame Rate:", this);
    frameRateSlider = new QSlider(Qt::Horizontal, this);
    frameRateSlider->setRange(30, 120);  // 设置帧率范围30-120
    frameRateSlider->setSingleStep(5);    // 设置步长为5
    frameRateLabel = new QLabel("60 FPS", this);
    frameRateLabel->setMinimumWidth(60);  // 设置固定宽度以避免跳动
    
    frameRateLayout->addWidget(frameRateTitleLabel);
    frameRateLayout->addWidget(frameRateSlider);
    frameRateLayout->addWidget(frameRateLabel);
    displayLayout->addLayout(frameRateLayout);
    
    // 连接帧率滑块的valueChanged信号，实时更新标签
    connect(frameRateSlider, &QSlider::valueChanged, this, [=](int value) {
        frameRateLabel->setText(QString::number(value) + " FPS");
    });
...
void SettingsWidget::updateUI()
{
    ...
    // 更新帧率设置
    frameRateSlider->setValue(currentSettings.frameRate);
    frameRateLabel->setText(QString::number(currentSettings.frameRate) + " FPS");
