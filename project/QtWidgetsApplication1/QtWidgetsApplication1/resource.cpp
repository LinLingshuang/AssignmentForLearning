...
{
    int index = maxHp - hp;
    // 添加数组边界检查和图片有效性检查，防止段错误
    if(index >= 0 && index < normalPic.size() && index < highlightPic.size()) {
        if(!isSelected && !normalPic[index].isNull())
            p->drawPixmap(0, 0, normalPic[index]);
        else if(isSelected && !highlightPic[index].isNull())
            p->drawPixmap(0, 0, highlightPic[index]);
    }
}
