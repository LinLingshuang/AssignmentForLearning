...
public:
    ...
    Rectangle(double w, double h);//往下查看set函数可以发现，Rectangle子类用于计算的是"w"和"h"，
                                  //且类型为double，不要遗漏";"
...
//Rectangle子类的析构函数，注意不要遗漏"{}"，且注意该行不是";"结尾！
    ...
    double getArea() const override;//面积。第三行的显示中已有实现方法的提示，即使用"override"，让Rectangle子类重载Shape基类中的getArea()函数，类型仍然是double，不要遗漏";"


    double getPerim() const override;//周长。方法同上一行。
...
public:
    ...
    Circle(double r);//下面set函数已提示为"double r"，不要遗漏";"
...
//对照Rectangle子类的实现方法，实现Circle子类的析构函数。注意事项同上。
    ...
    // 实现抽象方法
    // 
    double getArea() const override;//面积。对照Rectangle子类的实现方法，实现Circle子类的面积计算，不要遗漏";"


    double getPerim() const override;//周长。对照Rectangle子类的实现方法，实现Circle子类的周长计算，不要遗漏";"
