...
void MonsterSpawnerTower::spawnMonster()
{
    ...
    if (currentWave >= 15) {
        if (r < 10) {
            newMonster = new BatToPlayer();     // 10%
        }
        else if (r < 20) {
            newMonster = new BatToCamp();       // 10%
        }
        else if (r < 35) {
            newMonster = new Monster3();        // 15%
        }
        else if (r < 50) {
            newMonster = new Slime();            // 15%
        }
        else if (r < 80) {
            newMonster = new Monster2();        // 30%
        }
        else {
            newMonster = new Monster1();        // 20%
        }
    }
...
// 暂停所有怪物相关定时器
void MonsterSpawnerTower::pause()
{
    // 暂停所有定时器
    if (spawnTimer && spawnTimer->isActive()) {
        spawnTimer->stop();
...
// 恢复所有怪物相关定时器
void MonsterSpawnerTower::resume()
{
    // 恢复所有定时器
    if (spawnTimer) {
        spawnTimer->start(static_cast<int>(spawnInterval * 1000));
...
// 设置当前波数
void MonsterSpawnerTower::setCurrentWave(int wave)
{
    currentWave = wave;
    // 重新计算该波的怪物总数
    totalMonstersInWave = 5 + currentWave;
    monstersInCurrentWave = 0;
...
// 处理怪物死亡事件
void MonsterSpawnerTower::onMonsterDied(MonsterType type)
{
    // 这个函数目前为空，因为怪物死亡的处理已经在updateAllMonstersPosition中完成
    // 保留这个函数是为了兼容信号连接
}
