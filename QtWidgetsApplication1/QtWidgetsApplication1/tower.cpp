...
void Tower::put(int gridX, int gridY)
{
    ...
    // 设置格子状态为障碍物，防止重复放置
    if (gMap->isGridInsideMap(gridX, gridY)) {
        gMap->setTileState(gridX, gridY, TILE_BLOCKED);
    }
...
void Tower::pause()
{
    ...
    if (trackTimer && trackTimer->isActive()) {
        trackTimer->stop();
    }
}
...
{
    // 恢复所有定时器
    if (attackTimer && isActive && isPlaced) {
        attackTimer->start(attackInterval);
    }
    if (trackTimer && isActive && isPlaced) {
        trackTimer->start();
    }
}
